...
def analysis_group_view(request):
    ...
    if request.method == 'GET':
        ...
        analysisBy = []
        analysisBy.append('and year_of_birth != '' and year_of_birth IS NOT NULL order by year_of_birth')
        analysisBy.append('and level_of_education != '' and level_of_education IS NOT NULL order by level_of_education')
        analysisBy.append('')
        course = request.GET['course']
        request.GET['course'] = course
        to_render = {}
        ...
        # 影片觀看次數
        WatchCount = [0 for i in range(7)]
        WatchCount_output = ['' for i in range(7)]
        WatchCount_c = [0 for i in range(7)]

        # 討論區發文數
        forumNumber = [0 for i in range(7)]
        forumNumber_output = ['' for i in range(7)]
        forumNumber_c = [0 for i in range(7)]
        ...
        with connections['ResultDB'].cursor() as cursor:
            ...
            cursor.execute(
                "select max(run_date) as run_date from student_total_data0912 where course_id= %s", [courseId]
            )
            result = namedtuplefetchall(cursor)
            ...
            # 影片觀看次數
            count = 0
            for_watch_count = "SELECT watch_count,year_of_birth,level_of_education " \
                            "FROM student_total_data0912 " \
                            "WHERE course_id='" + courseId + "' " + "AND run_date='" + runDate_studentTotalData + "' " \
                            + analysisBy[int(select)]
            cursor.execute(for_watch_count)
            result = namedtuplefetchall(cursor)
            ...
            cursor.execute(for_forum_number)
            result = namedtuplefetchall(cursor)
            ...
            totalDataUseTable = ['student_total_data0912', 'student_total_data', 'student_total_data_old']
            ...
            cursor.execute(
                "SELECT avg(total_grade_best_p) as avgTotal_grade_best_p_scatter ,avg(forum_num) as avg_forum_num, "
                "avg(watch_count) as avgWatch_count_scatter "
            ...
            jsonArray_temp.append("觀看影片數目")
            jsonArray_temp.append("成績")
            ...
            jsonArray_tempForum.append("討論區發文數")
            jsonArray_tempForum.append("成績")
            ...
            count = 0
            for rs in result:
                watch_count_scatter = rs.watch_count_scatter
                total_grade_best_p_scatter = int(rs.total_grade_best_p_scatter * 100)

                StandardWatch = StandardWatch + preStandard(watch_count_scatter, avgWatch_count_scatter)
                StandardGrade = StandardGrade + preStandard(total_grade_best_p_scatter, avgTotal_grade_best)
                gradeWatch = gradeWatch + Correlation_X(watch_count_scatter, total_grade_best_p_scatter,
                                                        avgWatch_count_scatter, avgTotal_grade_best)
                count = count + 1
                jsonArray_temp.clear()
                jsonArray_temp.append(watch_count_scatter)
            ...
            for rs in result:
                ...
                if avgWatch_count_scatter + 2*StandardWatch > watch_count_scatter > avgWatch_count_scatter - 2*StandardWatch and avgTotal_grade_best + 2 * StandardGrade > total_grade_best_p_scatter > avgTotal_grade_best - 2 * StandardGrade:
                    jsonArray_temp.clear()

                    jsonArray_temp.append(watch_count_scatter)
                    jsonArray_temp.append(total_grade_best_p_scatter)
                    Standard_gradeWatch.append(jsonArray_temp.copy())
                    avgWatch_count_scatter_Correlation = avgWatch_count_scatter_Correlation + watch_count_scatter
            ...
            for rs in result:
                ...
                if avgWatch_count_scatter_Correlation + 2*StandardWatch > watch_count_scatter > avgWatch_count_scatter_Correlation - 2*StandardWatch and avgTotal_grade_best_Correlation + 2 * StandardGrade > total_grade_best_p_scatter > avgTotal_grade_best_Correlation - 2 * StandardGrade:
                    StandardCorrelation_WatchGrade = StandardCorrelation_WatchGrade + Correlation_X(
                        watch_count_scatter, total_grade_best_p_scatter, avgWatch_count_scatter_Correlation,
                        avgTotal_grade_best_Correlation
                    )

                    StandardCorrelation_Watch = StandardCorrelation_Watch + preStandard(
                        watch_count_scatter, avgWatch_count_scatter_Correlation
                    )
            ...
            for rs in result:
                forum_num_scatter = int(rs.forum_num)
                total_grade_best_p_scatter = int(rs.total_grade_best_p_scatter * 100)
                ...
                gradeForum = gradeForum + Correlation_X(
                    forum_num_scatter, total_grade_best_p_scatter, avgForum_num, avgTotal_grade_best
                )
                count = count + 1
            ...
            for rs in result:
                ...
                if avgForum_num_Correlation + 2*StandardForum > forum_num_scatter > avgForum_num_Correlation - 2*StandardForum and avgTotal_grade_Forum_Correlation + 2 * StandardGrade_Forum > total_grade_best_p_scatter > avgTotal_grade_Forum_Correlation - 2 * StandardGrade_Forum:
                    StandardCorrelation_ForumGrade = StandardCorrelation_ForumGrade + Correlation_X(
                        forum_num_scatter, total_grade_best_p_scatter, avgForum_num_Correlation,
                        avgTotal_grade_Forum_Correlation
                    )
            ...
            jsonArray_grade_count_scatter = json.dumps(jsonArray_grade_count_scatter)
            jsonArray_grade_Forum_scatter = json.dumps(jsonArray_grade_Forum_scatter)
            Standard_gradeWatch = json.dumps(Standard_gradeWatch)
            Standard_gradeForum = json.dumps(Standard_gradeForum)
            forumNumber = forumNumber_output
            WatchCount = WatchCount_output
            colume = column[int(select)]
            ...
            to_render['jsonArray_grade_count_scatter'] = jsonArray_grade_count_scatter  # 影片及分數分布圖
            ...
            to_render['avgTotal_grade_best_Correlation'] = '{:,.2f}'.format(avgTotal_grade_best_Correlation)  # 平均成績移除最大值
            to_render['degree_StandardWatchGrade'] = degree_StandardWatchGrade  # 相關程度(中文)
            to_render['Standard_gradeWatch'] = Standard_gradeWatch  # 影片及分數移除極大值後的圖
            to_render['StandardCorrelation_WatchGrade'] = '{:,.2f}'.format(StandardCorrelation_WatchGrade)  # 相關係數移除極大值

            # -----------------------------------------------發文數與成績分布圖
            to_render['avgForum_num'] = '{:,.2f}'.format(avgForum_num)  # 平均發文數
            to_render['avgTotal_grade_best'] = '{:,.2f}'.format(avgTotal_grade_best)  # 平均成績
            to_render['degree_ForumGrade'] = degree_ForumGrade  # 相關程度(中文)
            to_render['jsonArray_grade_Forum_scatter'] = jsonArray_grade_Forum_scatter  # 發文次數與成績圖
            ...
            to_render['Standard_gradeForum'] = Standard_gradeForum  # 發文次數與成績移除極大值後的圖
            ...
            to_render['runDate_studentTotalData'] = runDate_studentTotalData
            to_render['colume'] = column[int(select)]

        return render(request, 'AnalysisGroup.html', locals())
