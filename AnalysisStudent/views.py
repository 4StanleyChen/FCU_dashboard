    ...
    if request.method == 'GET':
        ...
        with connections['ResultDB'].cursor() as cursor:
            ...
            cursor.execute(
                ...
                "FROM " + totalDataUseTable[totalUse] + "as t_table INNER JOIN " + gradeUseTable[gradeUse] +
                " as g_table on (t_table.user_id=g_table.user_id) WHERE t_table.course_id='" + courseId +
                "'and g_table.course_id='" + courseId + "' and t_table.run_date='" + runDate +
                "' and g_table.run_date='" + runDate_grade + "' " + radio[select]
            )
            result = namedtuplefetchall(cursor)
            ...
            cursor.execute(
                "select 註冊人數+退選人數 as countPerson  from course_total_data_v2 where course_id= %s", [courseId]
            )
            result = namedtuplefetchall(cursor)
